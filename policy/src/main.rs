...
fn main() {
    let data_preparer = preparer::DataPreparer::new("data/montydata.binpack", 4096);
    ...
    //trainer.load_from_checkpoint("D:/montytrain/checkpoints/cpn_001-200");
...
fn eval_fen(trainer: &mut Trainer, output_node: Node, fen: &str) {
    use bullet::trainer::default::formats::montyformat::chess::{Castling, Move, Position};

    let mut castling = Castling::default();
    let pos = Position::parse_fen(fen, &mut castling);
    let mut data = loader::DecompressedData {
        pos,
        moves: [(0, 0); 108],
        num: 0,
    };
    ...
    for (mov, _) in &data.moves[..data.num] {
        ...
        println!(
            "{}: {}",
            mov.to_uci(&castling),
            output[moves::map_move_to_index(&pos, mov)]
        );
    }
}
